...
export default function Home() {
  ...
  const createNewChat = () => {
    const newChat = {
      ...
      messages: [
        {
          role: 'assistant',
          content: 'Pob tsawg! Tôi là AI H’Mông. Tôi có thể hỗ trợ bạn:\n\n• Dịch thuật & trò chuyện tiếng H’Mông - Việt\n• Giải toán, viết kịch bản video, phân tích dữ liệu\n• Bấm 📷 để tải ảnh lên phân tích, thay đổi bối cảnh hoặc gõ "vẽ..." để tạo ảnh AI.'
        }
      ]
    }
  ...
  const sendMessage = async (e) => {
    ...
    const userText = input || (selectedImage ? 'Phân tích và chỉnh sửa bức ảnh này giúp tôi' : '')
  ...
  return (
    <div className="flex h-screen bg-[#0f172a] text-slate-100 font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full relative bg-[#0f172a]">
        {/* Header với Logo AI H'Mông và nút Nâng Pro */}
        <header className="flex items-center justify-between p-3.5 bg-[#1e293b] border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2.5">
              <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-emerald-500 to-teal-400 flex items-center justify-center font-bold text-xs text-slate-950 shadow-md">H'M</div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => alert('Cổng thanh toán Nâng Pro 49k/tháng đang mở thử nghiệm!')} className="text-xs bg-amber-500/20 border border-amber-500/50 text-amber-300 px-3 py-1.5 rounded-lg font-medium hover:bg-amber-500/30 transition flex items-center gap-1 shadow-sm">
              <span>★</span>
        ...
        <main className="flex-1 overflow-y-auto p-4 space-y-4 max-w-3xl w-full mx-auto">
          {currentChat?.messages.map((msg, idx) => (
            <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[88%] p-3.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap ${msg.role === 'user' ? 'bg-emerald-600 text-white rounded-tr-none shadow-md' : 'bg-[#1e293b] text-slate-100 border border-slate-700/60 rounded-tl-none shadow-md'}`}>
          ...
          {loading && (
            <div className="flex justify-start">
              <div className="bg-[#1e293b] px-4 py-3 rounded-2xl text-xs text-slate-400 animate-pulse border border-slate-700/50">AI H’Mông đang xử lý...</div>
            </div>
          )}
        ...
        <footer className="p-3 bg-[#1e293b] border-t border-slate-800">
          ...
          <form onSubmit={sendMessage} className="max-w-3xl mx-auto flex gap-2">
            ...
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder="Nhập câu hỏi, bài toán, kịch bản hoặc yêu cầu tạo ảnh..." className="flex-1 bg-[#0f172a] border border-slate-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-emerald-500 transition" />
